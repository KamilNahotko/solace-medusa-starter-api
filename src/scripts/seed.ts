...
export default async function seedDemoData({ container }: ExecArgs) {
  ...
  const getCategoryIdsByName = (name: 'Shirts' | 'Sweatshirts' | 'Pants' | 'Merch'): string[] => {
    const { id } = categoryResult.find((cat) => cat.name === name) ?? {};
  ...
  const generateAdditionalProducts = (categoryResult, defaultSalesChannel) => {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      products.push({
        ...
        category_ids: getCategoryIdsByName(
          categoryName as 'Shirts' | 'Sweatshirts' | 'Pants' | 'Merch'
        ),
